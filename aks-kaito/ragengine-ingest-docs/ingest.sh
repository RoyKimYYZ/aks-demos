...
# Wrapper for ragengine-ingest-docs.py using a uv-managed venv.
#
# Examples:
#   export INGRESS_IP=1.2.3.4
#   ./ingest.sh --file ./docs/cra-tax-rules.txt --index rag_index --mode create
#   ./ingest.sh --file ./docs/cra-tax-rules.txt --index rag_index --mode update
#
# Or provide base URL directly:
#   ./ingest.sh --base-url http://1.2.3.4 --file ./docs/cra-tax-rules.txt --index rag_index
...
exec "${PY}" "${SCRIPT_DIR}/ragengine-ingest-docs.py" "$@"
...
uv run python ragengine-ingest-docs.py --base-url "http://$INGRESS_IP/rag" --file "$DOCUMENT_FILE" --index "$index_name" --mode update

# Update docs with max chars per document to chunk into smaller pieces. The benefit is more relevant for larger documents.
uv run python ragengine-ingest-docs.py --base-url "http://$INGRESS_IP/rag" --file "$DOCUMENT_FILE" --index "$index_name" --mode update --max-chars 800

# List documents in an index:
uv run python ragengine-ingest-docs.py --base-url "http://$INGRESS_IP/rag" --index "$index_name" --mode list   

uv run python ragengine-ingest-docs.py --base-url "http://$INGRESS_IP/rag" --index "$index_name" --mode list --metadata author=kaito --max-text-length 2000    

# Query/Ask a question against an index:
uv run python ragengine-ingest-docs.py --base-url "http://$INGRESS_IP/rag" --index "$index_name" --mode query \
