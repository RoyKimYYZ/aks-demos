...
Examples:
  export INGRESS_IP=1.2.3.4
        python3 ragengine-ingest-docs.py --file ./docs/cra-tax-rules.txt --index rag_index --mode create

        python3 ragengine-ingest-docs.py --file ./docs/cra-tax-rules.txt --index rag_index --mode update

    # List documents
        python3 ragengine-ingest-docs.py --index rag_index --mode list --limit 5 --offset 0 --max-text-length 500

    # Ask a question (RAG)
        python3 ragengine-ingest-docs.py --base-url http://$INGRESS_IP/rag --index rag_index --mode chat \
...
def request_json_with_retries(
    ...
    *,
    ...
    for attempt in range(1, retries + 1):
        try:
            if method_u == "POST":
                resp = requests.post(url, json=payload, timeout=(connect_timeout_s, timeout_s))
            else:
                resp = requests.get(url, params=params, timeout=(connect_timeout_s, timeout_s))
...
def _parse_metadata_inputs(
    ...
    if metadata_json:
        ...
        if not isinstance(obj, dict):
            raise click.ClickException("--metadata-json must be a JSON object")
        merged.update(obj)

    for item in metadata_items:
        if "=" not in item:
            raise click.ClickException(
                f"Invalid --metadata '{item}'. Use key=value (repeat for multiple keys)."
            )
        key, raw_val = item.split("=", 1)
        key = key.strip()
        if not key:
            raise click.ClickException(f"Invalid --metadata '{item}': key is empty")

        val_text = raw_val.strip()
        try:
            val: Any = json.loads(val_text)
        except Exception:
            val = val_text
        merged[key] = val
...
def _create_endpoint_path(base_url_eff: str) -> str:
    path = urlparse(base_url_eff).path.rstrip("/")
    if path and path != "":
        return "index"
    return "rag/index"
...
@click.option(
    ...
    "metadata_items",
    multiple=True,
    help="Create/update mode: metadata key=value (repeat for multiple keys)",
)
@click.option(
    "--metadata-json",
    default=None,
    help='Create/update mode: JSON object metadata, e.g. {"author":"kaito","year":2025}',
)
...
@click.option("--connect-timeout", type=int, default=5, show_default=True, help="HTTP connect timeout seconds")
...
def cli(
    ...
    if mode_l not in {"create", "update"} and (metadata_items or metadata_json):
        raise click.ClickException("--metadata/--metadata-json are only supported for create/update.")
    ...
    user_metadata = _parse_metadata_inputs(metadata_items, metadata_json)
    user_metadata["index_name"] = index
    ...
    if mode_l == "create":
        endpoint = urljoin(base_url_eff, _create_endpoint_path(base_url_eff))
        payload = {
            "index_name": index,
